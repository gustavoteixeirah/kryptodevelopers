...
contract("KryptoDevelopers", (accounts) => {
  ...
  describe("deployment", async () => {
    ...
    it("baseURI", async () => {
      ...
      assert.equal(name, "https://www.kryptodevelopers.dev/api/");
    });
    it("Price", async () => {
      const price = await contract.developerPrice();
      assert.equal(new BN(price).toString(), web3.utils.toWei("75", "ether"));

      await contract.setPrice(web3.utils.toWei("1", "ether"));
      const newPrice = await contract.developerPrice();
      console.log(newPrice);
      assert.equal(new BN(newPrice).toString(), web3.utils.toWei("1", "ether"));
    });
  });
  ...
  describe("basic functionalities", async () => {
    ...
    it("should not be able to mint more than 25 developers", async () => {
      await contract.mint(26, {
        from: accounts[1],
        value: web3.utils.toWei("26", "ether"),
      }).should.be.rejected;
    });
  });
  ...
  describe("minting", async () => {
    ...
    it("should be able to retrieve the token URI", async () => {
      ...
      assert.equal(tokenURI, "https://www.kryptodevelopers.dev/api/0");
    });
  ...
  describe("give away functionality", async () => {
    ...
    it("owner should be able to give away some nfts to a lucky account", async () => {
      const result = await contract.mintTo(luckyAccount, 5);
    ...
    it("lucky account should now have 5 tokens with ids 10 from 14", async () => {
      ...
      assert.equal(tokensOfOwner.toString(), [10, 11, 12, 13, 14].toString());
    });
  });
});
